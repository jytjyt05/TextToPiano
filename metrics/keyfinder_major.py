...
class Tonal_Fragment(object):
    ...
    def print_key(self,index):
        ...
        if self.altkey is not None:
            ...
            #     ListMajororMinor.append('Major%d.wav'%index)
            if ('major' in max(self.key_dict, key=self.key_dict.get) and 'minor' in self.altkey):
                ListMajororMinor.append('Major%d.wav'%index)
        if ('minor' in max(self.key_dict, key=self.key_dict.get) and self.altkey is None) or \
            ...
            ListMinorOnly.append('Major%d.wav'%index)
        if ('major' in max(self.key_dict, key=self.key_dict.get) and self.altkey is None) or \
                ('major' in max(self.key_dict, key=self.key_dict.get) and 'major' in self.altkey):
            ListMajorOnly.append('Major%d.wav'%index)
...
for i in range(41,42):
    ...
    audio_path = './Ours_Major/Major%d.png.wav'%i
    ...
    key_table = Tonal_Fragment(y_harmonic, sr, tend=5)
...
print(ListMajorOnly)
print(len(ListMajorOnly)+len(ListMajororMinor))
